...
<div class="flex flex-col justify-center items-center relative">
	...
	<div class="w-full text-center mb-4 flex  flex-col place-items-center">
		...
		<small
			>Please provide your <strong>email</strong> and <strong>password</strong> and {isSignIn
				? 'Log In'
				: 'Sign Up'}</small
		>
	</div>
	...
	<form class="w-full sm:w-1/2 xl:w-5/12" method="post" on:submit|preventDefault={signUpOrSignIn}>
		...
		>
			<button
				type="button"
				class="flex-1 bg-gray-200 text-green-700 py-3 rounded w-full text-center shadow"
				on:click|preventDefault={() => handleProviderSignIn('github')}
			>
				<GithubIcon size="1x" class="inline-block " />
			...
			<div class="mb-4">
				<label for="password" class="block font-semibold text-gray-800 mb-2 text-left"
					>Password</label
				>
			...
			<div class="flex pt-4 gap-2">
				<button
					type="submit"
					class="flex-1 bg-gray-500 border border-gray-600 text-white py-3 rounded w-full text-center shadow"
				>
