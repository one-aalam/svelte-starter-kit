	...
	async function getProfile() {
		try {
			...
			if (error instanceof TypeError) {
				...
				handleAlert({
					type: 'default',
					text: 'You know? You can click on the randomly generated avatar to update your profile picture.'
				});
			} else {
...
<div class="flex flex-col justify-center items-center relative">
	...
	<div class="profile-detail my-4" on:click={toggleModal}>
		...
		<span class="inline-block px-2 py-1 bg-gray-400 text-white rounded-full"
			><ChromeIcon class="inline-block" size="1x" /> {$profile.website}</span
		>
	...
	{#if user}
		...
		<div class="rounded-md shadow-2xl bg-green-800 w-3/5 overflow-hidden mt-4">
			...
			<small class="bg-gray-800 text-white px-4 py-2 w-full inline-block"
				>{JSON.stringify(user)}</small
			>
		</div>
	{/if}
</div>
